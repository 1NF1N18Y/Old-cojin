...
*Setup instructions for a Gitian build of Cojin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Cojin
...
- [Building Cojin Core](#building-cojin-core)
...
Clone the git repositories for cojin and Gitian.
...
in the cojin repository under 'Fetch and create inputs' to install sources which require
...
To build Cojin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the cojin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/cojin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/cojin-project/cojin
...
For example:
```bash
URL=https://github.com/thrasher-/cojin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit cojin=${COMMIT} --url cojin=${URL} ../cojin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit cojin=${COMMIT} --url cojin=${URL} ../cojin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit cojin=${COMMIT} --url cojin=${URL} ../cojin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the cojin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../cojin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/cojin
SIGPATH=/some/root/path/cojin-detached-sigs

./bin/gbuild --url cojin=${BTCPATH},signature=${SIGPATH} ../cojin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/cojin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/cojin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/cojin-osx-build.assert
```
...
[cojin-project/gitian.sigs.ltc](https://github.com/cojin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
