Cojin Core version 0.13.2 is now available from:

  <https://download.cojin.org/cojin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/cojin-project/cojin/issues>
...
updates will be released anymore. Without security updates, using a cojin
...
ECDSA signatures inside Cojin transactions now use validation using
...
Previous versions of Cojin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Cojin Core was to raise the default minimum
relay fee.

Cojin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Cojin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running cojind without having to do any manual configuration.
...
Cojin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Cojin Core will relay transactions with
...
entirely in Cojin Core 0.13, and it is at this time undecided whether the
...
Cojin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Cojin Core automatically creates a hidden service to listen on, without
manual configuration. Cojin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Cojin Core is listening, and
...
Cojind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Cojin
Core to use floating fees. By default, Cojin Core will use floating
fees.
...
(default: `0.0002` CJN/kB).

At all times, Cojin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) CJN.
Furthermore, Cojin Core will never create transactions paying less than
...
`cojin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /cojinrpc>
        ...
        # optional bypass cojind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< cojinrpc:<password>
    </Location>
...
Restarting cojind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in cojin.conf
...
For example:
    ...
    $ src/cojin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Cojin Core code base has
...
- `cojin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `cojin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Cojin scripting system that in combination with
...
Cojin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Cojin Core.
...
The two phases are distinct in the Cojin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Cojin wallet
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Cojin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of cojins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of cojins being spent is exactly
...
- **Increased security for multisig:** Cojin addresses (both P2PKH addresses
  ...
  believed to be provided by Cojin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Cojin's current ledger (called the UTXO set),
  ...
  note that Cojin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Cojin users to individually opt-in to almost any change in the Cojin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Cojin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Cojin
client to Cojin Core 0.13.2 to signal for segwit support. When a
...
Since Cojin Core 0.10.0, nodes have defaulted to only relaying and
...
- `cojin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `cojin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
