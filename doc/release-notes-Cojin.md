Cojin Core version *0.15.1* is now available from:

  <https://download.cojin.org/cojin-0.15.1/>
...
A number of changes to the way Cojin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 cojins per kB (0.00001 CJN). This is to prepare for dropping the minimum transaction fee to 0.1 cojins per kB (0.0001 CJN) in the next release.
